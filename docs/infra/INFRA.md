...
All jobs should be implemented in its own shell or python script. This shcript should be called from the pipeline. This makes sure there is separation between the pipeline definition (.yml files) and the implementation of each job, so that the two can vary independently.
