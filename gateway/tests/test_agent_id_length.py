...
Agent IDs longer than 128 characters should be rejected with 422.
"""
...
class TestAgentIdMaxLength:
    ...
    async def test_oversized_agent_id_rejected(self, client, api_key):
        """200+ char agent_id should return 422."""
        long_id = "a" * 200
        resp = await client.get(
    ...
    async def test_128_char_agent_id_accepted(self, client, api_key):
        ...
        exact_id = "b" * 128
        # This will 403 because the agent doesn't exist/doesn't match, but NOT 422
        resp = await client.get(
    ...
    async def test_129_char_agent_id_rejected(self, client, api_key):
        """129 chars should be rejected with 422."""
        long_id = "c" * 129
        resp = await client.get(
            f"/v1/billing/wallets/{long_id}/balance",
            headers={"Authorization": f"Bearer {api_key}"},
