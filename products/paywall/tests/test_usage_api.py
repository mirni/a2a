...
class TestUsageSummary:
    ...
    async def test_summary_with_usage(
        ...
        await tracker._storage.record_usage(
            agent_id="agent-1", function="test_fn", cost=5.0
        )
        await tracker._storage.record_usage(
            agent_id="agent-1", function="test_fn", cost=3.0
        )
    ...
    async def test_summary_defaults_to_free_without_keys(
        self, usage_api: UsageAPI
    ):
...
class TestUsageHistory:
    ...
    async def test_history_with_records(self, usage_api: UsageAPI, tracker):
        await tracker._storage.record_usage(
            agent_id="agent-1", function="fn1", cost=1.0
        )
        await tracker._storage.record_usage(
            agent_id="agent-1", function="fn2", cost=2.0
        )
    ...
    async def test_history_with_time_filter(self, usage_api: UsageAPI, tracker):
        await tracker._storage.record_usage(
            agent_id="agent-1", function="old_fn", cost=1.0
        )
        now = time.time()
        await tracker._storage.record_usage(
            agent_id="agent-1", function="new_fn", cost=2.0
        )
    ...
    async def test_history_limit(self, usage_api: UsageAPI, tracker):
        for i in range(10):
            await tracker._storage.record_usage(
                agent_id="agent-1", function=f"fn{i}", cost=1.0
            )
...
class TestProjectedCost:
    async def test_projected_no_usage(
        self, usage_api: UsageAPI, key_manager: KeyManager
    ):
        await key_manager.create_key(agent_id="agent-1", tier="pro")
    ...
    async def test_projected_with_usage(
        ...
        for _ in range(10):
            await tracker._storage.record_usage(
                agent_id="agent-1", function="fn", cost=1.0
            )
    ...
    async def test_projected_free_tier(
        self, usage_api: UsageAPI, key_manager: KeyManager
    ):
        await key_manager.create_key(agent_id="agent-1", tier="free")
