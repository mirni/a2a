...
class ScopeChecker:
    ...
    def check_scope(self, tool_name: str) -> None:
        ...
        if required not in self.effective_scopes:
            raise KeyScopeError(
                f"Key scope insufficient: tool '{tool_name}' requires '{required}' scope"
            )
    ...
    def check_tool(self, tool_name: str) -> None:
        ...
        if self.allowed_tools is not None and tool_name not in self.allowed_tools:
            raise KeyScopeError(
                f"Tool '{tool_name}' not in allowed tools for this key"
            )
    ...
    def check_agent_id(self, agent_id: str) -> None:
        ...
        if self.allowed_agent_ids is not None and agent_id not in self.allowed_agent_ids:
            raise KeyScopeError(
                f"Agent '{agent_id}' not in allowed agent_ids for this key"
            )
