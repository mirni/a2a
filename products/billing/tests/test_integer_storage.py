...
class TestIntegerStorage:
    async def test_wallet_balance_stored_as_integer(self, storage):
        ...
        cursor = await storage.db.execute("SELECT balance, typeof(balance) FROM wallets WHERE agent_id = 'int-test'")
    ...
    async def test_usage_cost_stored_as_integer(self, storage):
        ...
        cursor = await storage.db.execute("SELECT cost, typeof(cost) FROM usage_records WHERE agent_id = 'cost-test'")
        row = await cursor.fetchone()
        assert row[1] == "integer", f"Expected integer, got {row[1]} (value={row[0]})"
    ...
    async def test_transaction_amount_stored_as_integer(self, storage):
        ...
        cursor = await storage.db.execute("SELECT amount, typeof(amount) FROM transactions WHERE agent_id = 'tx-test'")
        row = await cursor.fetchone()
        assert row[1] == "integer", f"Expected integer, got {row[1]} (value={row[0]})"
    ...
    async def test_precision_no_float_loss(self, storage):
        ...
        cursor = await storage.db.execute("SELECT balance FROM wallets WHERE agent_id = 'precision-test'")
        row = await cursor.fetchone()
