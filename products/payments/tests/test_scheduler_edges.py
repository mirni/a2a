...
from payments.engine import PaymentEngine, InvalidStateError
...
class TestProcessDueEdges:
    ...
    async def test_insufficient_balance_handled_gracefully(
        self, scheduler, engine, billing_wallet
    ):
        ...
        sub = await engine.create_subscription(
            payer="poor-payer", payee="payee-x", amount=100.0,
            interval="hourly",
        )
        # Make it due
    ...
    async def test_multiple_subscriptions_due_simultaneously(
        self, scheduler, engine, funded_wallets
    ):
    ...
    async def test_suspended_subscription_not_processed(
        self, scheduler, engine, funded_wallets
    ):
    ...
    async def test_next_charge_at_updated_after_success(
        self, scheduler, engine, funded_wallets
    ):
